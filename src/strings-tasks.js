...
function sumOfCodes(str) {
  // Check if str is undefined or not a string, return 0 in those cases
...
function formatTime(minutes, seconds) {
  ...
  return `${addNuls(minutes)}:${addNuls(seconds)}`;
}
...
function isPalindrome(str2) {
  const str = str2.replace(/[^a-zA-Z]/g, '');
  const reversed = [...str].reverse().join('').toLowerCase().trim();
  const s = [...str].join('').toLowerCase().trim();
...
function findLongestWord(sentence) {
  ...
  sentence.split(' ').forEach((e) => {
    if (e.length > world.length) world = e;
  });
  return world;
}
